...
Section Pouet2.
  ...
  Context {Z} (a: B -> Z) (b: C -> Z) (c: a o f == b o g).
  Context `{h: Z -> Z'}.
  Let PO := pushout f g.
  Definition F2 : PO -> Z.
  Proof.
  ...
  Proof.
    refine (pushout_rec _ _ _).
    destruct 1; auto.
    intro; cbn; by apply ap.
  Defined.

  Goal h o F2 == G2.
    refine (pushout_ind _ _ _ _ _).
    - intro x. destruct x; exact (Eq_to_paths E1).
    - intro x. cbn.
      rewrite transport_paths_FlFr.
      rewrite ap_compose.
      unfold F2, G2; rewrite !pushout_rec_beta_pp.
  Abort.
End Pouet2.
  ...
  Lemma LP_C_AF `(f : A -> A') `(g : B' -> B) (Hf : IsCoFibration f) (Hg : IsSurjectiveEquivalence g) : LP f g.
    ...
    - intro x. transitivity (β (α x)).
      + generalize (α x); clear α x.
        ...
        (* transparent assert (bla: B). { *)
        (*   clear β. *)
        (*   refine (pushout_rec B _ _ p). *)
        (*   destruct 1. exact (g b). exact (g (s (G (y; base y)))). *)
        (*   intro a; cbn. apply ap. exact (((H2 (F a))^ @ ap s (Eq_to_paths (H a))) @ *)
        (*    ap s (ap G (path_sigma' (λ y : Y, Cyl k y) 1 (cyl_eq a)))). } *)
        (* transitivity bla. admit. subst bla. *)
        (* subst pt β. apply Eap10. clear p. *)
        (* refine (Eap011D _ _ _). *)
        (* apply eq_forall. intro z; destruct z. reflexivity. *)
        (* by symmetry. *)
        ...
        (*  with *)
        (*  | inl b => E1 *)
        (*  | inr y => (H1 (G (y; base y)))^E *)
        (*  end)). *)
        (* set (p := (ap g *)
        (* (((H2 (F x))^ @ ap s (Eq_to_paths (H x))) @ *)
        (*  ap s (ap G (path_sigma' (λ y : Y, Cyl k y) 1 (cyl_eq x)))))). *)
        (* transitivity (concat_EVpE (e (inl (F x))) p (e (inr (k x)))). *)
        (* admit. subst p e. cbn. rewrite ap_pp. (* !! *) *)
        (* rewrite concat_EVpE_pp. cbn. *)
        (* rewrite <- ap_compose.  (* !!! *) *)
        (* rewrite ap_pp, ap_V.    (* !!! *) *)
        (* clear surj_H4; assert (H4: forall x, ap g (H2 x) ≡ concat_EVp (H1 _) 1). admit. *)
        (* rewrite H4. destruct (H1 (g (F x))). *)
